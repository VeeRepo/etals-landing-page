...
onMounted(() => {
  ...
  function movePoints(time: number) {
    lines.forEach((pts) => {
      pts.forEach((p: Point) => {
        const move =
          noise.perlin2((p.x + time * props.waveSpeedX) * 0.002, (p.y + time * props.waveSpeedY) * 0.0015) * 12
...
<template>
  <div
    ...
    :class="cn('absolute top-0 left-0 w-full h-full overflow-hidden', className)"
  >
    <div
      :class="cn('absolute top-0 left-0 rounded-full', 'w-2 h-2 bg-foreground/10')"
